...
public:
    ...
    struct BattMonitor_State {
        ...
        float       total_mwh;           // total energy draw since start-up
...
#define _BattMonitor_STATE(instance) state[instance]
    ...
    /// total_mwh - returns total energy drawn since start-up in amp-hours
    float total_mwh(uint8_t instance) const;
    float total_mwh() const { return total_mwh(AP_BATT_PRIMARY_INSTANCE); }
